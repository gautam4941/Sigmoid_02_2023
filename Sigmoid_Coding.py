...
word = """elon Reeve CoviD Musk FRabcS (/ˈiːlɒn/ EE-lon; born June 28, 1971) is a business magnate and investor. He is the founder, CEO and chief engineer of SpaceX; angel investor, CEO and product architect of Tesla, Inc.; owner and CEO of Twitter, Inc.; founder of The Boring Company; co-founder of Neuralink and OpenAI; and president of the philanthropic Musk Foundation. With an estimated net worth of around $139 billion as of December 23, 2022, primarily from his ownership stakes in Tesla and SpaceX,[4][5] Musk is the second-wealthiest person in the world, according to both the Bloomberg Billionaires Index and Forbes's real-time billionaires list.[6][7]
...
# ^ -> Start or Not
# . -> Anything
# \symbol -> Will Search only given specific character
# .{3} -> Consider 3 repeating any character
# * -> 0 or more Occurance
# + -> 1 or more Occurance
...
# print( re.findall( '[a-z]+', word ) )
# print( re.findall( '[A-Z]+', word ) )
# print( re.findall( '[A-Z]+.{3}[A-Z]', word ) )
...
# print( re.findall( '^[A-Z].+[A-Z]', word )  )
# print()
# print( re.match( '[A-Z].+[A-Z]', word )  )

# functions in RE
# findall() -> return the matching string
# match()   -> Check if the matching string is present at start or not
# split()   -> Split the string on the basis of matching pattern
# sub()     -> Substitute the string on the basis of matching pattern
# search()  -> Check if the matching string is present anywhere in string or not

# word = "Elon Reeve CoviD Musk FRabcS (/ˈiːlɒn/ EE-lon; born June 28, 1971) is a bu"
...
# print()
# print( re.search( '\d', word ) )  

import re

# 1)
data ='Ram is 22, and Rohit is 33, Dinesh is 45 and Shwetha is 23'

# Find out Single Digit, All digits having atleast 1 or more occurance

#1) a )
# print( re.findall( '\d', data ) )   # ['2', '2', '3', '3', '4', '5', '2', '3']

# 1) b )
# print( re.findall( '\d+', data ) )   # ['22', '33', '45', '23']

# 2) Find out all the names
...
# 3) Find out the phone numbers from given string
# print( re.findall( '\d+-\d+-\d+', phn ) )

# 4) Find out all the number individually and get their average
"""
digits = re.findall( '\d', phn )
print( digits )
...
print( f"Add = { add } and Average = { add/len( digits ) }" )
"""

# 5) Find out all the number in continious manner and get their average
"""
digits = re.findall( '\d+', phn )
print( digits )
...
word = """from STEPHEN.MARQUARD@UCT.AC.ZA SAT JAN
RETURN-PATH: 456 <P_OSTMASTER@COLLAB.SAKAIPROJECT.ORG>
RECEIVED: FROM MURDER (MAIL.UMICH.EDU [])
BY FRANKENSTEIN.MAIL.UMICH.EDU (CYRUS ) WITH LMTPA;
SAT, JAN
YOU CAN"""

# 6) Find out all the upper cases and replace it by lower case
# print( re.findall( '[A-Z]+', word ) )
# print( word.lower() )

# 7) Find out email id from the given string
# print( re.findall( '[A-Z|a-z]+[\d|\w|_|\.]+@[A-Z|a-z]+[\.|A-Z|a-z]+', word ) )

# 8) Find out all the continuous special character and replace it by
# print( re.sub( '\W+', '*', word ) )

data = """Sachin Ramesh Tendulkar (/ˌsʌtʃɪn tɛnˈduːlkər/ (About this soundlisten); born 24 April 1973) is an Indian former international cricketer and a former captain of the Indian national team. He is widely regarded as one of the greatest batsmen in the history of cricket.[4] He is the highest run scorer of all time in International cricket. Tendulkar took up cricket at the age of eleven, made his Test debut on 15 November 1989 against Pakistan in Karachi at the age of sixteen, and went on to represent Mumbai domestically and India internationally for close to twenty-four years.
...
Tendulkar received the Arjuna Award in [5] [1994] for his outstanding sporting achievement, the Rajiv Gandhi Khel Ratna award in 1997, India's highest sporting honour, and the Padma Shri and Padma Vibhushan awards in 1999 and 2008, respectively, India's fourth and second highest civilian awards.[16] After a few hours of his final match on 16 November 2013, the Prime Minister's Office announced the decision to award him the Bharat Ratna, India's highest civilian award.[17][18] He is the youngest recipient to date and the first ever sportsperson to receive the award.[19][20].
...
# print( re.findall( '[a-z]+', data ) )
# print( re.findall( '[A-Z][a-z]+', data ) )
# print( re.findall( '[A-Z]{2}[a-z]+', data ) )

"""
for i in re.findall( '\d{4}', data ):
      if( int( i ) > 2005 ):
            print( i, end = ', ' )
"""
...
def square( data ):
      ...
      for i in data:
            print( f"Square, i = {i} and sq(i) = { i**2 }, Thread Name = { threading.current_thread().name }\n" )
            time.sleep( 0.5 )

def cube( data ):

      for i in data:
            print( f"Cube, i = {i} and cube(i) = { i**2 }, Thread Name = { threading.current_thread().name }\n" )
            time.sleep( 2 )
...
t1 = threading.Thread( target = square, args = ( data, ), name = "T1"  )
t2 = threading.Thread( target = cube, args = ( data, ), name = "T2" )
...
# print( list( zip( name, age, roll ) ) )

for i in zip( name, age, roll ):
      print( i )
"""
...
print( f"Total = {reduce( add, numbers )}" )
"""
...
def findEven( num ):
      print( f"findEven(), num = { num }" )
      return num%2 == 0

print( list( filter( findEven, numbers ) ) )
"""


# import json

# file_path = r"C:\Users\gauta\PycharmProjects\Python_Concept_Gautam\Json\DUMMY_JSON_DATA_3.json"
"""
f = open( file_path, "r" )
data = json.load( f )
...
"""
def findTitle( dict_data ):
      # print( f"findTitle() : dict_data :- \n{ dict_data }\n" )
      
      return title in dict_data["title"]

title = "offici"
op = filter( findTitle, data )
...
def findTitle( dict_data ):
      # print( f"dict_data :- \n{ dict_data }\n" )
      return dict_data["albumId"] == dict_data["id"]

for i in filter( findTitle, data ):
      print( i )
"""

"""
def findDuplicateTitle( dict_data ):
      isPresent = dict_data["title"] in uniqueTitle
      
      if( dict_data["title"] not in uniqueTitle ):
            uniqueTitle.append( dict_data["title"] )
            
      return isPresent

uniqueTitle = []
for i in filter( findDuplicateTitle, data ):
      print( i )
"""

"""
def findDuplicateThumbnailUrl( dict_data ):
      isPresent = dict_data["thumbnailUrl"] in uniqueThumbnailUrl
      
      if( dict_data["thumbnailUrl"] not in uniqueThumbnailUrl ):
            uniqueThumbnailUrl.append( dict_data["thumbnailUrl"] )
            
      return isPresent

uniqueThumbnailUrl = []
for i in filter( findDuplicateThumbnailUrl, data ):
      print( i , "\n")
...
'''
# Decorator 
def add_one(func): # func = add
      print( f"add_one : func = { func }\n" ) # add_one : func = add
      
      def wrapper(x):
            print("In add_one wrapper, before calling func")
            result = func(x) + 1 # result = add(x) + 1
            print( f"In add_one wrapper, after calling func, result = {result}\n", )
...
@add_one                # add = add_one.wrapper(x)
@add_two                # add = add_two.wrapper(x)
@add_three              # add = add_three.wrapper(x)
def add(x):
    return x

print( f"Outside : add = { add }\n" )
print( add(0) ) # add = wrapper(0) = result
...
executeQuery( "use mamta" )
# executeQuery( "Show Tables" )

# Read
# executeQuery( "select * from employee LIMIT 5" )

# executeQuery( """update employee set proj_id = 'P4' where 1=1 and proj_id = 'P3'""" )
# executeQuery( """DELETE FROM employee where 1=1 and proj_id = 'P4'""" )
executeQuery( """INSERT INTO employee( emp_id, emp_name, manager, salary, proj_id )
                  values( 1412071662, 'M11', 'M1', 20000, 'P4' )""" )
...
"""
pds1 = pd.Series( range( 1, 6 ) )
pds2 = pd.Series( range( 6, 11 ) )
pds3 = pd.Series( range( 11, 16 ) )
pds4 = pd.Series( range( 16, 21 ) )
...
# pds1 = pd.Series( index = [ 'a', 'b', 'c', 'd', 'e' ], data = range( 1, 6 ) )
pds1 = pd.Series( index = [ 'a', 'b', 'c', 'd', 'e' ], data = [1, 1.2, True, "Hello", [11, 12]] )
print( f"Before, pds1 :- \n{ pds1 }\n" )
...
'''
pds1 = pd.Series( data = range( 1, 6 ) )
pds2 = pd.Series( data = range( 6, 11 ) )
pds3 = pd.Series( data = range( 11, 16 ) )
pds4 = pd.Series( data = range( 16, 20 ) )
...
# d = [ pds1, pds2, pds3, pds4 ]
# pdf1 = pd.DataFrame( d ).T

pdf1 = pd.DataFrame( d )
print( f"Before, pdf1 :- \n{pdf1}\n" )

# pdf1['col1'][2] = 4
# print( f"pdf1['col4'] + 10 :- \n{ pdf1['col4'] + 10 }\n" )
# print( f"pdf1['col4'] + 'hello' :- \n{ pdf1['col4'] + 'hello' }\n" )  -> Error
# print( f"pdf1['col3'] + pdf1['col4'] :- \n{ pdf1['col3'] + pdf1['col4'] }\n" )
# print( f"pdf1['col3'] * pdf1['col4'] :- \n{ pdf1['col3'] * pdf1['col4'] }\n" )
# print( f"pdf1*10 :- \n{ pdf1*10 }\n" )
# print( f"pdf1.loc[ 2, : ] * 10 :- \n{ pdf1.loc[ 2, :  ] * 10 }\n" )

# print( pdf1%2 == 0 )
# print( pdf1>10 )
# print( (pdf1>7) | ( pdf1<15) )
# print( pdf1[ (pdf1>7) | ( pdf1<15) ] )

pdf1['col5'] = [21, 22, 23, 24, 25]
# pdf1.insert(2, 'col5', [21, 22, 23, 24, 25] )

# print( pdf1.pop( 'col5' ) )
# del pdf1['col5']
# pdf1.drop( [2, 4], axis = 0, inplace = True ) # axis = 1 : Col and axis = 0 : row
...
# csv_data.fillna( 5, inplace = True )
